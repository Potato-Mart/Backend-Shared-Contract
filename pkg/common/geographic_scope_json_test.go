...
func TestGeographicScopeAndContextJSON(t *testing.T) {
	scope := GeographicScope{
		...
		Targets: []GeographicTarget{
			{Kind: geographyenum.GeographicTargetCountry, Code: "AU"},
			{Kind: geographyenum.GeographicTargetDepot, Code: "AU-VIC-MEL-DC-01"},
		},
	}
	...
	context := GeographicContext{
		...
		MatchedTargetKind:  geographyenum.GeographicTargetDepot,
...
func TestGlobalFallbackGeographicContextOmitsUnresolvedProfileGeography(t *testing.T) {
	payload, err := json.Marshal(GeographicContext{
		Source:             geographyenum.GeographicContextSourceGlobalFallback,
		ScopeRevision:      2,
		RuleRevision:       4,
