...
import (
	...
	"github.com/Potato-Mart/Backend-Shared-Contract/v36/pkg/contracts/orders/cart"
	orderfulfilment "github.com/Potato-Mart/Backend-Shared-Contract/v36/pkg/contracts/orders/fulfilment"
	sales "github.com/Potato-Mart/Backend-Shared-Contract/v36/pkg/contracts/orders/order"
	...
	"github.com/Potato-Mart/Backend-Shared-Contract/v36/pkg/contracts/pricing/market/market_enums"
	"github.com/Potato-Mart/Backend-Shared-Contract/v36/pkg/contracts/supply/catalogue/product/product_enums"
)
...
func TestOrderBuyerAndItemPricingRoundTrip(t *testing.T) {
	...
	order := sales.Order{
		...
		Items: []sales.OrderItem{
			{
				Components:     []orderfulfilment.PricedPackageComponent{{RequestedPackageCount: 10, RequestedBaseUnits: 10, PackagePrice: money.Money{AmountMinor: 8000, Currency: "AUD"}}},
				TotalBaseUnits: 10,
				Pricing: &buyer.PricingContext{
					Audience:   market_enums.PriceAudienceWholesale,
	...
	if len(decoded.Items) != 1 || decoded.Items[0].Pricing == nil || decoded.Items[0].Pricing.Audience != market_enums.PriceAudienceWholesale {
...
func TestCartChannelAndBuyerRoundTrip(t *testing.T) {
	...
	shoppingCart := cart.Cart{
		...
		Buyer:     &buyer.BuyerContext{Type: retail_enums.BuyerTypeGuestRetail},
		Items: []cart.CartItem{
			{
				Components:         []orderfulfilment.PricedPackageComponent{{RequestedPackageCount: 1, RequestedBaseUnits: 1, PackagePrice: money.Money{AmountMinor: 500, Currency: "AUD"}}},
				TotalBaseUnits:     1,
				Pricing:            &buyer.PricingContext{Audience: market_enums.PriceAudienceRetail},
	...
	payload, err := json.Marshal(shoppingCart)
	if err != nil {
		t.Fatalf("marshal cart: %v", err)
	}

	var decoded cart.Cart
	if err := json.Unmarshal(payload, &decoded); err != nil {
	...
	if len(decoded.Items) != 1 || decoded.Items[0].Pricing == nil || decoded.Items[0].Pricing.Audience != market_enums.PriceAudienceRetail {
...
func TestBuyerCommercialTargetMappings(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			order:       sales.Order{Channel: commerce_enums.OrderTypeB2B, Buyer: &buyer.BuyerContext{Type: retail_enums.BuyerTypeWholesaleOrganisation}},
			wantChannel: "b2b",
		...
		},
		{
			...
			order: sales.Order{
				Channel: commerce_enums.OrderTypePOS,
				Buyer:   &buyer.BuyerContext{Type: retail_enums.BuyerTypeWholesaleOrganisation},
				Items:   []sales.OrderItem{{Pricing: &buyer.PricingContext{Audience: market_enums.PriceAudienceWholesale}}},
			},
			wantChannel:  "pos",
			wantBuyer:    "wholesale_organisation",
		...
		},
		{
			...
			order:       sales.Order{Channel: commerce_enums.OrderTypePOS, Buyer: &buyer.BuyerContext{Type: retail_enums.BuyerTypeGuestRetail}},
			wantChannel: "pos",
			wantBuyer:   "guest_retail",
		},
		{
			name:        "retail website member",
			order:       sales.Order{Channel: commerce_enums.OrderTypeOnline, Buyer: &buyer.BuyerContext{Type: retail_enums.BuyerTypeRetailCustomer}},
			wantChannel: "online",
