...
import (
	...
	"github.com/Potato-Mart/Backend-Shared-Contract/v34/pkg/contracts/orders/shipping/shipping_enums"
...
func TestOrderSummaryJSONShape(t *testing.T) {
		...
		Items: []sales.OrderLineSummary{
			{
				...
				ProductImage: &security.ObjectMedia{Code: "media_1", URL: "https://cdn.example.test/products/A0001.png"},
				ProductPackageOption: product.ProductPackageOption{
					Code: "EACH", SKUCode: "A0001",
	...
	if len(decoded.Items) != 1 || decoded.Items[0].SKUCode != "A0001" || decoded.Items[0].ProductName != "Potato 1kg" ||
		decoded.Items[0].ProductImage == nil || decoded.Items[0].ProductImage.Code != "media_1" || decoded.Items[0].ProductImage.URL != "https://cdn.example.test/products/A0001.png" ||
		decoded.Items[0].ProductPackageOption.Code != "EACH" || !decoded.Items[0].CapturedAt.Equal(now) ||
	...
	for _, removed := range []string{"product_sku_code", "name", "image", "image_url", "components", "accepted_package_pricing", "offer"} {
