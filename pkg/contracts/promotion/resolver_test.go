...
func targetedPromo(id string, class enums.PromotionClass, scope enums.DiscountScope, ref string) Promotion {
	p := Promotion{
		ID:    id,
		Name:  id,
...
func TestResolveNormalAppliesWithoutSpecial(t *testing.T) {
	active := []Promotion{targetedPromo("prm_normal", enums.PromotionClassNormal, enums.DiscountScopeProduct, "prd_pork_belly")}
...
func TestResolveCategoryPromotionAppliesToMember(t *testing.T) {
	active := []Promotion{targetedPromo("prm_cat", enums.PromotionClassNormal, enums.DiscountScopeCategory, "pork")}
...
func TestResolveCategoryPromotionDescendants(t *testing.T) {
	...
	if eff := ResolveEffective([]Promotion{p}, porkBelly()); eff == nil || eff.PromotionID != "prm_meat" {
		t.Fatalf("effective = %+v, want prm_meat (descendant match)", eff)
	}
	off := false
	p.TargetIncludesDescendants = &off
	if eff := ResolveEffective([]Promotion{p}, porkBelly()); eff != nil {
...
func TestResolvePriorityTieBreak(t *testing.T) {
	...
	b.Priority = 5
	if eff := ResolveEffective([]Promotion{a, b}, porkBelly()); eff == nil || eff.PromotionID != "prm_b" {
...
func TestDiscountedUnitPriceMinor(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			p := Promotion{DiscountSpec: DiscountSpec{DiscountType: tc.dtype, DiscountValue: tc.value}}
