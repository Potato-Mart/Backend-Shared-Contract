...
func TestRetailOrderItemJSONPreservesMixedCaseAndEachPricing(t *testing.T) {
	...
	item := sales.OrderItem{
		...
		TotalBaseUnits:     27,
		SubstitutionPolicy: sales.LooseSubstitutionPolicySnapshot{Allowed: true, Source: salesenum.LooseSubstitutionPolicySourceBuyerSelected, CapturedAt: now},
...
func TestGroupOrderFulfilmentJSONUsesOneParentAllocation(t *testing.T) {
	...
	parent := sales.Order{OrderNumber: "PARENT-1", GroupOrder: &sales.GroupOrderContext{GroupOrderCode: "GROUP-1", Role: salesenum.GroupOrderRoleConsolidatedParent, ParentFulfilmentID: "fulfilment_1"}, GroupOrderFulfilment: &plan}
...
func offerSnapshot(id string, option product.ProductPackageOptionSnapshot, price common.Money, capturedAt time.Time) product.SellableOfferSnapshot {
	return product.SellableOfferSnapshot{
		...
		GeographicContext: common.GeographicContext{Source: geographyenum.GeographicContextSourceRetailCustomerProfile, CountryCode: "AU", ScopeRevision: 1, RuleRevision: 3, EvaluationTimezone: "Australia/Melbourne"},
	}
}
