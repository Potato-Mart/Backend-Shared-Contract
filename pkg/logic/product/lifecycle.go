...
const (
	// DisplayStatusNew marks an active product still inside its NEW (æ–°å“)
	...
	// DisplayStatusOutOfStock marks an active product with no sellable stock (ç¼ºè²¨).
...
// background job, and cannot drift â€” the same instant always produces the same
// answer everywhere. All timestamps are stored and compared in UTC.
const (
	// LifecycleTagNew marks a æ–°å“: listed for the first time within the last
	// LifecycleTagTTL.
	LifecycleTagNew = "NEW"
	// LifecycleTagRestocked marks a è£œè²¨ product: total sellable stock went
	// 0 â†’ >=1 within the last LifecycleTagTTL.
...
// through, while an active product resolves to new â†’ restocked â†’ out_of_stock â†’
...
// automatically â€” no background job.
