...
func TestRewardRedemptionFreezesOptionalMarketAndCountry(t *testing.T) {
	now := time.Date(2026, 8, 24, 0, 0, 0, 0, time.UTC)
	redemption := reward.RewardRedemption{
		ID:             "rr-1",
		CustomerNumber: "RC-1",
	...
	emptyPayload, err := json.Marshal(reward.RewardRedemption{})
	if err != nil {
...
func TestCheckoutBenefitReservationExcludesPersistenceRetryKey(t *testing.T) {
	...
	checkoutReservation := reservation.CheckoutBenefitReservation{
	...
	if _, exists := reflect.TypeOf(reservation.CheckoutBenefitReservation{}).FieldByName("IdempotencyKey"); exists {
...
func TestRewardRedemptionOutcomeCarriesTypedIssueEvidence(t *testing.T) {
	...
	redemption := reward.RewardRedemption{
		...
		PointsSpent:    5000,
		Status:         wallet_enums.RewardRedemptionStatusRedeemed,
		Outcome: &reward.RewardRedemptionOutcome{
			GiftCardCode: "gc-001",
			External: &reward.ExternalRewardFulfilment{
	...
	var decoded reward.RewardRedemption
	...
	bare, err := json.Marshal(reward.RewardRedemption{})
	if err != nil {
		t.Fatalf("marshal empty reward redemption: %v", err)
	}
