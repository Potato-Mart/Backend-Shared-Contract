...
try {
    ...
    $versionMatch = [regex]::Match(
        $moduleSource,
        '(?m)^//\s*contract-release:\s*(?<version>v\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?)\s*$'
    )
    Assert-Match $versionMatch.Success 'go.mod does not contain a valid contract-release declaration.'
    ...
    Assert-Match $semverMatch.Success "contract-release '$version' is not valid release semver."
    ...
    ) "go.mod module path '$moduleImportPath' does not match contract-release '$version'."

    if ($ExpectedVersion) {
        Assert-Match (
            $ExpectedVersion -eq $version
        ) "Requested release '$ExpectedVersion' does not match source contract-release '$version'."
    }

    $versionLinePattern = "(?m)^$([regex]::Escape($version))\s*$"
    Assert-Match (
        [regex]::IsMatch($readmeSource, $versionLinePattern)
    ) "README.md does not advertise source contract-release '$version'."
